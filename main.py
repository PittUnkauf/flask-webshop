...
def root():
    ...
    searchQuery = '%' + str(request.args.get('searchQuery') or '') + '%'
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        cur.execute('SELECT productId, name, price, description, image, stock FROM products WHERE name LIKE ?', [searchQuery])
